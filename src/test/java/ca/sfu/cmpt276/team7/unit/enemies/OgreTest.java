...
class OgreTest {
	...
	// Ensure the ogre follows the exact route, reversing when necessary, instead of looping around
	assertTrue(ogre.getPosition().equals(new Position(0, 0)));
	ogre.updateMovement(playerPos);
	assertTrue(ogre.getPosition().equals(new Position(1, 0)));
	ogre.updateMovement(playerPos);
	...
	// This is messy, but it reduces the use of other classes in unit tests
	Board board = new Board(new Cell[][] {
		{ new FloorCell(new Position(0, 0)), new FloorCell(new Position(1, 0)), new WallCell(new Position(2, 0)), new FloorCell(new Position(3, 0))},
	});
